"""Round 15: Model Diversity (3XGB + 6LGB + 3CB + 1HGB)

Adds HGB model and expands CatBoost to 3 seeds for greater diversity.
Uses Hill Climbing ensemble selection with optimized weights.

Pipeline:
1. Load data + pairwise TE (135 cols) + TE_ORIG (19 cols)
2. Stage 1: Train 13 models (3 XGB + 6 LGB + 3 CB + 1 HGB) without pseudo labels
3. Pseudo-label test set from Stage 1 average (threshold=0.90)
4. Stage 2: Retrain all 13 models WITH pseudo-labeled data (weight=0.5x)
5. Ensemble: Hill Climbing + LR stacking + simple avg, take best
...
start = time.time()
log("=" * 60)
log("Round 15: Model Diversity (3XGB + 6LGB + 3CB + 1HGB)")
log("=" * 60)
...
def train_hgb(train_df, y_arr, test_df, te_cols, seed, nm, n_orig=None, pw=None):
    ...
    for fold, (tri, vai) in enumerate(skf.split(train_df, y_arr)):
        ...
        feat = [c for c in X_tr.columns if c != TARGET]
        # HGB needs numeric features only - drop categoricals
        for c in CATS:
            for df_ in [X_tr, X_va, X_te]:
...
# ============================================================
# STEP 5: Stage 1 — Train 13 models without pseudo labels
# ============================================================
...
# 5d: HistGradientBoosting (1 model)
...
# Hill Climbing (greedy forward selection)
log("  Running Hill Climbing...")
hc_names = []
hc_best_score = 0
hc_remaining = list(names)
...
log(f"  Hill Climbing selected {len(hc_names)} models, OOF: {hc_best_score:.5f}")
log(f"  Selected: {hc_names}")

# Optimize weights for selected models with scipy
from scipy.optimize import minimize
...
opt_wa = -res_w.fun
log(f"  Hill Climbing + optimized weights OOF: {opt_wa:.5f}")
...
log(f"  >>> Using {best_method} (score: {scores[best_method]:.5f})")
...
fname = SUBMISSIONS / "submission_r15_thresh_opt.csv"
sub.to_csv(fname, index=False)
dist = pd.Series([rmap[p] for p in preds_thresh]).value_counts()
log(f"  r15_thresh_opt: {dict(dist)}")
...
fname2 = SUBMISSIONS / "submission_r15_ens_default.csv"
sub2.to_csv(fname2, index=False)
dist2 = pd.Series([rmap[p] for p in preds_default]).value_counts()
log(f"  r15_ens_default: {dict(dist2)}")

# Hill Climbing weighted
...
fname_hc = SUBMISSIONS / "submission_r15_hill_climbing.csv"
sub_hc.to_csv(fname_hc, index=False)
dist_hc = pd.Series([rmap[p] for p in preds_hc]).value_counts()
log(f"  r15_hill_climbing: {dict(dist_hc)}")
...
for n in list(s2_oof.keys()):
    ...
    log(f"    {n}: {s:.5f}")
log(f"  Simple avg OOF: {sa_score:.5f}")
log(f"  Hill Climbing OOF: {hc_best_score:.5f}")
log(f"  Hill Climbing + optimized OOF: {opt_wa:.5f}")
log(f"  Stacked OOF: {ssc:.5f}")
