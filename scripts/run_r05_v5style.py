"""Round 05: V5-style Full Pipeline

Based on analysis of previous V5 (LB=0.97745):
1. All-pairwise TE (NUMS+CATS, all C(19,2)=171 combos, filter high cardinality)
2. TE_ORIG (original data groupby mean for each column)
3. sklearn.preprocessing.TargetEncoder (within-fold, not manual K-Fold)
4. 10-model ensemble (3 XGB + 6 LGB + 1 CB) with multiple seeds/hyperparams
5. Stacking with LR meta-learner
6. Threshold optimization (2 params: Medium and High weights)
"""
import warnings
...
start = time.time()
log("=" * 60)
log("Round 05: V5-style Full Pipeline")
log("=" * 60)
...
for SEED in SEEDS:
    ...
    for fold, (tri, vai) in enumerate(skf.split(train, y)):
        ...
        model = xgb.XGBClassifier(
            ...
            callbacks=[xgb.callback.EarlyStopping(rounds=100, metric_name="bal_ACC", maximize=True, save_best=True)],
            eval_metric=make_bal_acc(),
            max_bin=1024, random_state=SEED, n_jobs=-1, tree_method="hist"
        )
        model.fit(X_tr[feat], y[tri], eval_set=[(X_va[feat], y[vai])], sample_weight=sw, verbose=False)
    ...
    for SEED in [42]:
        ...
        for fold, (tri, vai) in enumerate(skf.split(train, y)):
            ...
            model = cb_mod.CatBoostClassifier(
                ...
                colsample_bylevel=0.8, l2_leaf_reg=3.0, min_data_in_leaf=50, random_seed=SEED
            )
            model.fit(X_tr[feat], y[tri], sample_weight=sw,
...
sub_opt = pd.DataFrame({"id": tids, TARGET_COL: [rmap[p] for p in preds_opt]})
path_opt = SUBMISSIONS / "submission_r05_thresh_opt.csv"
sub_opt.to_csv(path_opt, index=False)
log(f"  r05_thresh_opt: {dict(sub_opt[TARGET_COL].value_counts())}")

# Default ensemble (no threshold opt)
preds_default = bt.argmax(axis=1)
sub_default = pd.DataFrame({"id": tids, TARGET_COL: [rmap[p] for p in preds_default]})
path_default = SUBMISSIONS / "submission_r05_ens_default.csv"
sub_default.to_csv(path_default, index=False)
log(f"  r05_ens_default: {dict(sub_default[TARGET_COL].value_counts())}")
