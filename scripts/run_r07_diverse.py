"""Round 07: Diverse Ensemble — No Orig Fusion

Based on R06 finding: orig fusion (20x) did NOT improve XGB (0.97713 vs R05's 0.97718).
Conclusion: orig data signal is already captured by TE_ORIG features.

Strategy: Focus on model diversity instead of data augmentation.
1. Same 135 pairwise TE features as R05
2. Same TE_ORIG features
3. More diverse XGB configs: 3 seeds × 2 hyperparam sets = 6 XGB models
4. Tuned LightGBM with 2 configs × 3 seeds = 6 LGB models
5. 2 CatBoost models (different depths)
6. Total: 14 models → stacking → threshold opt

Bet: More diverse models in stacking = better generalization = higher LB.
"""
import warnings
...
start = time.time()
log("=" * 60)
log("Round 07: Diverse Ensemble (14 models)")
log("=" * 60)
...
# === 5a. XGBoost: 2 hyperparam configs × 3 seeds = 6 models ===
log("\n  --- XGBoost (2 configs × 3 seeds = 6) ---")
xgb_configs = [
    {"max_depth": 6, "subsample": 0.8, "colsample_bytree": 0.8,
     "n_estimators": 5000, "learning_rate": 0.03, "max_bin": 1024},
    {"max_depth": 8, "subsample": 0.7, "colsample_bytree": 0.7,
     "n_estimators": 3000, "learning_rate": 0.04, "max_bin": 512},
]
for xi, xcfg in enumerate(xgb_configs):
    for SEED in SEEDS_XGB:
        ...
        for fold, (tri, vai) in enumerate(skf.split(train, y)):
            X_tr, X_va, X_te = train.iloc[tri].copy(), train.iloc[vai].copy(), test.copy()
            X_tr, X_va, X_te = apply_te(X_tr, X_va, X_te, TE_columns, y[tri])

            classes = np.unique(y[tri])
            cw = dict(zip(classes, compute_class_weight("balanced", classes=classes, y=y[tri])))
            sw = np.array([cw[l] for l in y[tri]])
            ...
            model.fit(X_tr[feat], y[tri], eval_set=[(X_va[feat], y[vai])],
...
# === 5b. LightGBM: 2 configs × 3 seeds = 6 models ===
log("\n  --- LightGBM (2 configs × 3 seeds = 6) ---")
lgb_configs = [
...
for li, lp in enumerate(lgb_configs):
    for SEED in SEEDS_LGB:
        ...
        for fold, (tri, vai) in enumerate(skf.split(train, y)):
            ...
            model = lgb.LGBMClassifier(**lp, random_state=SEED)
            model.fit(X_tr[feat], y[tri], sample_weight=sw,
                      eval_set=[(X_va[feat], y[vai])],
...
# === 5c. CatBoost: 2 models (depth 6 + depth 8) ===
log("\n  --- CatBoost (2 configs) ---")
try:
    ...
    cb_configs = [
        ...
        {"iterations": 600, "learning_rate": 0.07, "depth": 8,
         "colsample_bylevel": 0.7, "l2_leaf_reg": 5.0, "min_data_in_leaf": 30},
    ]
    for ci, ccfg in enumerate(cb_configs):
        SEED = 42
        nm = f"cat{ci}_s{SEED}"
        skf = StratifiedKFold(n_splits=NF, shuffle=True, random_state=SEED)
        oof = np.zeros((len(train), 3))
        ...
        for fold, (tri, vai) in enumerate(skf.split(train, y)):
            ...
            model = cb_mod.CatBoostClassifier(
                **ccfg, task_type="CPU", auto_class_weights="Balanced",
