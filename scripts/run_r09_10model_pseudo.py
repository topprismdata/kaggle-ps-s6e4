"""Round 09: 10-Model Ensemble + Pseudo-Labeling

Combines the best of R05 (10-model diversity, stacking) with R08 (pseudo-labeling).
Key hypothesis: model diversity AND pseudo-labeling should stack additively.

Pipeline:
1. Load data + pairwise TE (135 cols) + TE_ORIG (19 cols)
2. Stage 1: Train 10 models (3 XGB + 6 LGB + 1 CB) without pseudo labels
3. Pseudo-label test set from Stage 1 average (threshold=0.90)
4. Stage 2: Retrain all 10 models WITH pseudo-labeled data (weight=0.5x)
5. Fast ensemble: weighted avg (2000 iter) + LR stacking, take best
...
start = time.time()
log("=" * 60)
log("Round 09: 10-Model + Pseudo-Labeling")
log("=" * 60)
...
# ============================================================
# STEP 5: Stage 1 — Train 10 models without pseudo labels
# ============================================================
...
fname = SUBMISSIONS / "submission_r09_thresh_opt.csv"
sub.to_csv(fname, index=False)
dist = pd.Series([rmap[p] for p in preds_thresh]).value_counts()
log(f"  r09_thresh_opt: {dict(dist)}")
